...
pub struct VoiceConfig {
    ...
    pub talking_threshold_db: f32,
    pub yelling_threshold_db: f32,
...
pub struct AudioStatus {
    pub running: bool,
    pub level_db: f32,
    pub raw_level_db: f32,
    pub processed_level_db: f32,
    pub voice_state: &'static str,
...
pub struct AudioEngine {
    ...
    raw_level: Arc<AtomicU32>,
    processed_level: Arc<AtomicU32>,
    voice: Arc<AtomicU8>,
    device_name: Arc<Mutex<String>>,
    suppression_enabled: Arc<AtomicBool>,
    suppression_strength: Arc<Mutex<String>>,
}
impl Default for AudioEngine {
    fn default() -> Self {
        Self {
            ...
            processed_level: Arc::new(AtomicU32::new((-100.0f32).to_bits())),
            voice: Arc::new(AtomicU8::new(0)),
            device_name: Arc::new(Mutex::new(String::new())),
            suppression_enabled: Arc::new(AtomicBool::new(false)),
...
impl AudioEngine {
    ...
    pub fn start(&self, mut config: VoiceConfig, app: AppHandle) -> Result<(), String> {
        ...
        let raw = self.raw_level.clone();
        let processed = self.processed_level.clone();
        let voice = self.voice.clone();
        let running = self.running.clone();
        let device_name = self.device_name.clone();
        let suppression_enabled = self.suppression_enabled.clone();
        let suppression_strength = self.suppression_strength.clone();
        let configured_suppression = config.noise_suppression_enabled;
        let configured_strength = config.suppression_strength.clone();
        std::thread::Builder::new()
            .name("livesprite-audio".into())
            .spawn(move || {
                let result = initialize_stream(
                    config,
                    app,
                    raw.clone(),
                    processed.clone(),
                ...
                match result {
                    Ok((stream, name)) => {
                        if let Ok(mut stored) = device_name.lock() {
                            *stored = name;
                        }
                        if let Ok(mut stored) = suppression_strength.lock() {
                            *stored = configured_strength;
                        }
                ...
                voice.store(0, Ordering::Release);
                raw.store((-100.0f32).to_bits(), Ordering::Relaxed);
                processed.store((-100.0f32).to_bits(), Ordering::Relaxed);
            })
            .map_err(|e| e.to_string())?;
        match init_rx
        ...
        {
            Ok(()) => {
                *self.stop.lock().map_err(|_| "Audio engine unavailable")? = Some(stop_tx);
                Ok(())
            }
            Err(error) => Err(error),
        }
    ...
    pub fn stop(&self) {
        ...
        self.voice.store(0, Ordering::Release);
        self.raw_level
            .store((-100.0f32).to_bits(), Ordering::Relaxed);
        self.processed_level
            .store((-100.0f32).to_bits(), Ordering::Relaxed);
    }
    pub fn status(&self) -> AudioStatus {
        ...
        AudioStatus {
            ...
            level_db: processed,
            raw_level_db: f32::from_bits(self.raw_level.load(Ordering::Relaxed)),
            processed_level_db: processed,
...
fn validate_config(config: &mut VoiceConfig) -> Result<(), String> {
    if !(config.noise_gate_db < config.talking_threshold_db
        && config.talking_threshold_db < config.yelling_threshold_db)
    {
        return Err("Noise Gate must be below Talking, and Talking must be below Yelling.".into());
    }
    if !config.smoothing.is_finite() || !(0.0..=0.98).contains(&config.smoothing) {
        return Err("Smoothing must be between 0 and 0.98.".into());
    }
    if !config.sensitivity.is_finite() || !(0.25..=4.0).contains(&config.sensitivity) {
        return Err("Sensitivity must be between 0.25 and 4.0.".into());
    }
    config.suppression_strength = match config.suppression_strength.to_ascii_lowercase().as_str() {
        "low" => "low",
        "medium" => "medium",
        "high" => "high",
        "very_high" | "very high" | "veryhigh" => "very_high",
...
fn initialize_stream(
    ...
    let device = if let Some(name) = config.device_name.as_deref().filter(|v| !v.is_empty()) {
        host.input_devices()
            ...
            .ok_or("The selected microphone is no longer available.")?
    } else {
    ...
    let supported = select_stream_config(&device, config.noise_suppression_enabled)?;
...
fn select_stream_config(
    ...
    if !suppression {
        return device
            ...
            .map_err(|e| format!("Unable to open microphone: {e}"));
    }
...
where
    T: cpal::SizedSample + ToLevel,
{
    let channels = usize::from(config.channels);
    let sample_rate = config.sample_rate.0;
    let error_app = app.clone();
    let error_raw = raw.clone();
    let error_processed = processed.clone();
    let error_voice = voice.clone();
...
fn process_level(detector: &mut Detector, mono: &[f32], sample_rate: u32) -> f32 {
    ...
    if detector.config.noise_suppression_enabled {
        ...
        detector
            .pending
            .extend(mono.iter().map(|sample| sample.clamp(-1.0, 1.0) * 32_768.0));
        while detector.pending.len() >= FRAME_SIZE {
            let input = detector.pending.drain(..FRAME_SIZE).collect::<Vec<_>>();
            let mut output = [0.0f32; FRAME_SIZE];
            detector.denoiser.process_frame(&mut output, &input);
            let wet = suppression_mix(&detector.config.suppression_strength);
            let mixed = input
                .iter()
                .zip(output.iter())
                .map(|(dry, clean)| ((dry * (1.0 - wet)) + (clean * wet)) / 32_768.0)
                .collect::<Vec<_>>();
    ...
    detector.smoothed_db = if detector.smoothed_db <= -99.0 {
        measured
    } else {
        detector.smoothed_db * smoothing + measured * (1.0 - smoothing)
    };
    detector.smoothed_db
}
fn suppression_mix(strength: &str) -> f32 {
    match strength {
        "low" => 0.35,
        "high" => 0.82,
        "very_high" => 1.0,
...
fn rms_db(samples: &[f32]) -> f32 {
    ...
    (sum / samples.len() as f32).sqrt().max(0.00001).log10() * 20.0
}
fn classify(db: f32, current: u8, c: &VoiceConfig) -> u8 {
    if db < c.noise_gate_db {
        return 0;
    }
    let h = 3.0;
    if current == 2 && db >= c.yelling_threshold_db - h {
        return 2;
    }
    if db >= c.yelling_threshold_db {
        return 2;
    }
    if current >= 1 && db >= c.talking_threshold_db - h {
        return 1;
    }
    if db >= c.talking_threshold_db {
        return 1;
    }
    0
}
...
impl ToLevel for f32 {
    fn level(&self) -> f32 {
        *self
    }
}
impl ToLevel for i16 {
    fn level(&self) -> f32 {
        *self as f32 / i16::MAX as f32
    }
}
impl ToLevel for u16 {
    fn level(&self) -> f32 {
...
mod tests {
    ...
    fn config() -> VoiceConfig {
        VoiceConfig {
            device_name: None,
            noise_gate_db: -55.0,
            talking_threshold_db: -38.0,
            yelling_threshold_db: -18.0,
            noise_suppression_enabled: true,
            suppression_strength: "medium".into(),
            smoothing: 0.72,
            sensitivity: 1.0,
            activation_ms: 80,
            release_ms: 220,
    ...
    fn thresholds() {
        let c = config();
        assert_eq!(classify(-60.0, 0, &c), 0);
        assert_eq!(classify(-50.0, 0, &c), 0);
        assert_eq!(classify(-30.0, 0, &c), 1);
        assert_eq!(classify(-10.0, 1, &c), 2);
    }
    #[test]
    fn controls() {
        let mut c = config();
        ...
        c.smoothing = 1.2;
        assert!(validate_config(&mut c).is_err());
    }
    #[test]
    fn strengths() {
        assert!(suppression_mix("low") < suppression_mix("medium"));
        assert!(suppression_mix("medium") < suppression_mix("high"));
        assert!(suppression_mix("high") < suppression_mix("very_high"));
    }
}
